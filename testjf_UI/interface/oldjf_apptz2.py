    ...
    def test_Login(self):
        ...
        for d in datas:
            with self.subTest (data=d):
                ...
                # #对带sign的地址在此包装
                # a2 = Mysign.mysign (content_tz2)
                # print (a2)
                # m.update (a2.encode (encoding='UTF-8'))  # 生成加密串，其中password是要加密的字符串
                # # hashlib.md5('abcdefg'.encode(encoding='UTF-8')).hexdigest()
                # sign2 = m.hexdigest ()
                # print (sign2)
                # content_tz3 = {'access_token': '1b1d5757-9322-4d83-bb79-fed79f973dad',
                #                'amount': '100',
                #                'biddCode': code,
                #                'investType': '1',
                #                'sessionId': sessionid[0],
                ...
                #                'userCode': usercode[0]
                #                }
                # rtz = requests.post ("http://192.168.1.249:9921/hkjfapp/invest/invest",params=content_tz3, cookies=c)
                # print('投资状态',rtz.status_code)
                # print(rtz.text)
